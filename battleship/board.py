...
class Board:
    ...
    def are_ship_lengths_correct(self): 
        ...
        """
        #NOTE: the length is the key and the frequency [number] is the value
    ...
    def are_ships_too_close(self):
        ...
        for ship in self.ships:
            #Iterate through the index of each ship in the ships list
            for index in range(len(self.ships)):
                #In the case that the ship instance is the same as that summoned by the 2nd loop, move on to the next iteration using continue
                if self.ships[index] == ship:
                    continue
                else:
                    #Returns true the first instance it finds a ship that is close to another ship in the ships list. This is the IFF condition
                    if ship.is_near_ship(self.ships[index]):
                        return True
        #In the case there are no pairs of ships that are close to each other, return False
        return False
    ...
    def have_all_ships_sunk(self):
        ...
        """
        #Create a list of sunk ships
        sunk_ships_list = []
        #iterate through the list of ship instances
        for ship in self.ships:
            #Check if each ship in the ships list has sunk and if so, append the ship to the sunk_ships_list
            if ship.has_sunk():
                sunk_ships_list.append(ship)
        #When the length of the sunk_ships_list equals that of the ships_list, all ships have sunk. Function returns True
        if len(sunk_ships_list) == len(self.ships):
            return True
        #If the lengths of the lists are not equivalent, return False. Not all ships have sunk    
        else:
            return False
    ...
    def is_attacked_at(self, cell):
        ...
        for ship in self.ships:
            #Check if the cell ckecked against is present in the cells occupied by the current ship instance in the iterable
            #Check if ship has been hit. In this case is_ship_hit will always be True because of the conditional statement preceding it
            is_ship_hit = ship.receive_damage(cell)
            #Inthe case the current ship instance has not sunk yet, has_ship_sunk will always yield False
            has_ship_sunk = ship.has_sunk()
                #Return statement outputs (True, False) in this case. The attack did not cause the ship to sink
            if is_ship_hit:
                return (is_ship_hit, has_ship_sunk)
        #If the cell being checked against is not occupied by the ships in the ships list, return (False, False). Ship not hit and ship has not sunk
        return (False, False)
